...
export const rawToRai = (raw: string | number): number => {
  ...
  return value.shiftedBy(16 * -1).toNumber();
};
...
export const ACCOUNT_REGEX = /((nano|xrb|usd|xusd)_)?[13][13-9a-km-uw-z]{59}/;
...
export const getPrefixedAccount = (address: string) => {
  ...
  if (!address.includes("_")) {
    account = `xusd_${address}`;
  } else if (address.startsWith("xrb_")) {
    account = address.replace("xrb_", "xusd_");
  }
  return account;
};
